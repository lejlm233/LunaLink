...
export abstract class InteractionHandler {
    ...
    protected static buildTouchOnClient(event: CommonTouchAndMouse, screenInfo: ScreenInfo): TouchOnClient | null {
        ...
        // 旋转适配：显示层（video-layer / touch-layer）被 CSS rotate 后，getBoundingClientRect
        // 返回的是旋转后的轴对齐包围盒（AABB），而 clientWidth/clientHeight 仍是未旋转布局尺寸。
        // 需把 AABB 内坐标逆旋转回未旋转盒子坐标，触摸才能正确映射到设备坐标。
        const angle = InteractionHandler.getCssRotationDeg(target);
        if (angle !== 0) {
            const ax = touchX - rect.width / 2;
            const ay = touchY - rect.height / 2;
            const rad = (-angle * Math.PI) / 180;
            const cos = Math.cos(rad);
            const sin = Math.sin(rad);
            const lx = ax * cos - ay * sin;
            const ly = ax * sin + ay * cos;
            touchX = clientWidth / 2 + lx;
            touchY = clientHeight / 2 + ly;
        }
        let invalid = false;
    ...
    // 读取元素实际 CSS 旋转角（度，顺时针为正）。用于把旋转后 AABB 内的触摸坐标逆旋转回视频本地坐标。
    private static getCssRotationDeg(el: HTMLElement): number {
        ...
        if (parts.length < 4 || !Number.isFinite(parts[0]) || !Number.isFinite(parts[1])) {
            return 0;
        }
        // CSS rotate(θ): matrix(cosθ, sinθ, -sinθ, cosθ, 0, 0) → θ = atan2(b, a)
