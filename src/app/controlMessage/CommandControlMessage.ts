...
export class CommandControlMessage extends ControlMessage {
    ...
    public static Commands: Map<number, string> = new Map([
        ...
        [ControlMessage.TYPE_REQUEST_KEY_FRAME, '请求关键帧'],
    ...
    public static createRequestKeyFrameCommand(): CommandControlMessage {
        return new CommandControlMessage(ControlMessage.TYPE_REQUEST_KEY_FRAME);
    }

    public static createGetClipboardCommand(): CommandControlMessage {
        // 官方 scrcpy v3.x 格式：[type][copyKey(1)]
        const event = new CommandControlMessage(ControlMessage.TYPE_GET_CLIPBOARD);
        const buffer = Buffer.alloc(1 + 1);
        buffer.writeUInt8(event.type, 0);
        buffer.writeUInt8(ControlMessage.COPY_KEY_NONE, 1);
        event.buffer = buffer;
        return event;
    }

    public static createSetClipboardCommand(text: string, paste = false): CommandControlMessage {
        // 官方 scrcpy v3.x 格式：[type][sequence(8)][paste(1)][textLength(4)][text]
