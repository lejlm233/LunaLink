...
export class DeviceTracker extends BaseDeviceTracker<GoogDeviceDescriptor, never> {
    ...
    protected buildDeviceRow(tbody: Element, device: GoogDeviceDescriptor): void {
        ...
        // 1. Control group: PID + Interface select + Update button (one desc-block)
        const controlGroupTd = document.createElement('div');
        controlGroupTd.classList.add('control-group', blockClass);
        services.appendChild(controlGroupTd);
        DESC_COLUMNS.forEach((item) => {
            ...
            if (fieldName === 'pid') {
                ...
                controlGroupTd.appendChild(actionButton);
                ...
                // 防御：个别 descriptor 可能缺失 interfaces 字段，直接 forEach 会抛 TypeError
                // 导致整个设备行渲染失败。
                ...
                controlGroupTd.classList.add('hidden');
                /// #endif
                if (isActive) {
                    ...
                    // 离线设备没有任何接口信息，也不存在 proxy over adb 选项。
                    // 空 <select> 在部分移动浏览器上点击不会弹出、宽度还会塌陷，
                    // 表现为"下拉栏点不开"——渲染禁用的占位项并禁用整个控件。
                    const placeholder = document.createElement('option');
                    placeholder.innerText = '设备离线';
                ...
                controlGroupTd.appendChild(selectElement);
            } else {
                // Fallback: any other field renders as its own desc-block
        ...
        // 2. Tools & stream group: list files + Configure stream + players (one desc-block)
        const toolsGroupTd = document.createElement('div');
        toolsGroupTd.classList.add('tools-group', blockClass);
        services.appendChild(toolsGroupTd);
        const flattenEntry = (entry: HTMLElement | DocumentFragment): void => {
            const nodes = entry instanceof DocumentFragment ? Array.from(entry.childNodes) : [entry];
            nodes.forEach((node) => {
                if (node instanceof HTMLElement && node.classList.contains(blockClass)) {
                    // Wrapper desc-block div — extract its children into the tools group
                    while (node.firstChild) {
                        toolsGroupTd.appendChild(node.firstChild);
                    }
                } else {
                    toolsGroupTd.appendChild(node);
                }
            });
        };
        // 2a. Tools: shell, devtools, list files (flatten each tool's desc-block wrapper)
        DeviceTracker.tools.forEach((tool) => {
            const entry = tool.createEntryForDeviceList(device, blockClass, this.params);
            if (entry) {
                if (Array.isArray(entry)) {
                    entry.forEach((item) => {
                        item && flattenEntry(item);
                    });
                } else {
                    flattenEntry(entry);
                }
        ...
        if (DeviceTracker.CREATE_DIRECT_LINKS) {
            ...
            players.forEach((playerClass) => {
                ...
                toolsGroupTd.appendChild(playerTd);
            });
        }
