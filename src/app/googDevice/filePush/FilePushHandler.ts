import { DragAndDropHandler, DragEventListener, DroppedFile } from '../DragAndDropHandler';
...
export default class FilePushHandler implements DragEventListener {
    ...
    /** 设备端目标目录（上传的根目录），由页面组件随当前浏览路径同步 */
    private targetDir = '/data/local/tmp';
    ...
    public setTargetDir(dir: string): void {
        if (!dir) {
            return;
        }
        this.targetDir = dir.endsWith('/') ? dir.slice(0, -1) : dir;
    }
    ...
    private async pushFile(file: File, destPath: string): Promise<void> {
    ...
    public onFilesDrop(dropped: DroppedFile[]): boolean {
        this.listeners.forEach((listener) => {
            listener.onDrop();
        });
        dropped.forEach(({ file, relativePath }) => {
            ...
            // 保持目录结构：拼出设备端目标路径（过滤 "." / ".." 等穿越段）
            const safeRelative = relativePath
                .split(/[\\/]+/)
                .filter((segment: string) => segment && segment !== '.' && segment !== '..')
                .join('/');
            const destPath = safeRelative ? `${this.targetDir}/${safeRelative}` : `${this.targetDir}/${file.name}`;
            this.pushFile(file, destPath);
        });
        return true;
    }
