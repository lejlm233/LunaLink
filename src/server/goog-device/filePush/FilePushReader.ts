...
export class FilePushReader {
    ...
    private async createStream(chunk: Buffer): Promise<void> {
        ...
        // 支持子目录上传：目标父目录不存在时先创建（如 /data/local/tmp/photos/）
        await this.ensureTargetDir(client);
    ...
    /** 目标文件位于子目录时，通过 adb shell 先建好目录（mkdir -p） */
    private async ensureTargetDir(client: ExtendedClient): Promise<void> {
        const idx = this.fileName.lastIndexOf('/');
        if (idx <= 0) {
            return;
        }
        const dir = this.fileName.slice(0, idx);
        if (!dir || dir === '.') {
            return;
        }
        const escaped = dir.replace(/'/g, `'\\''`);
        try {
            const stream = await client.shell(this.serial, `mkdir -p '${escaped}'`);
            await AdbExtended.util.readAll(stream);
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            console.error(`mkdir -p failed (${this.serial} | ${dir}): ${message}`);
        }
    }
