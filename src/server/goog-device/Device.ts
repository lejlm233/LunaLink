...
export class Device extends TypedEmitter<DeviceEvents> {
    ...
    private brightnessModeAuto: boolean | null = null;
    ...
    public async turnOffScreen(): Promise<string> {
        const results: string[] = [];

        // Step 1: Try brightness approach first
        try {
            const brightResult = await this.runShellCommandAdb('settings get system screen_brightness');
            const brightness = parseInt(brightResult.trim(), 10);
            if (!isNaN(brightness) && brightness > 0) {
                this.savedBrightness = brightness;
            }
            const modeResult = await this.runShellCommandAdb('settings get system screen_brightness_mode');
            this.brightnessModeAuto = modeResult.trim() === '1';
            if (this.brightnessModeAuto) {
                await this.runShellCommandAdb('settings put system screen_brightness_mode 0');
            }
            const setResult = await this.runShellCommandAdb('settings put system screen_brightness 0');
            results.push(`brightness=0: ${setResult.trim() || 'ok'}`);
        } catch (e) {
            results.push(`brightness failed: ${(e as Error).message}`);
        }

        // Step 2: Also try input keyevent 26 (power press) as fallback
        try {
            const powerResult = await this.runShellCommandAdb('input keyevent 26');
            results.push(`keyevent 26: ${powerResult.trim() || 'ok'}`);
        } catch (e) {
            results.push(`keyevent 26 failed: ${(e as Error).message}`);
        }

        // Step 3: Try dim screen via content provider (works on some devices)
        try {
            const contentResult = await this.runShellCommandAdb(
                'content insert --uri content://settings/system --bind name:s:screen_brightness --bind value:i:0',
            );
            if (!contentResult.includes('Error') && !contentResult.includes('Permission')) {
                results.push(`content: ${contentResult.trim()}`);
            }
        } catch (_e) {
            // silently skip
        }
    ...
    public async wakeUpScreen(): Promise<string> {
        const results: string[] = [];

        // Step 1: Restore brightness if we saved it
        if (this.savedBrightness !== null) {
            try {
                const targetBrightness = this.savedBrightness;
                if (this.brightnessModeAuto !== null && this.brightnessModeAuto) {
                    await this.runShellCommandAdb('settings put system screen_brightness_mode 1');
                }
                const result = await this.runShellCommandAdb(
                    `settings put system screen_brightness ${targetBrightness}`,
                );
                results.push(`brightness=${targetBrightness}: ${result.trim() || 'ok'}`);
                this.savedBrightness = null;
                this.brightnessModeAuto = null;
            } catch (e) {
                results.push(`brightness restore failed: ${(e as Error).message}`);
            }
        }

        // Step 2: Wake up screen
        try {
            const wakeResult = await this.runShellCommandAdb('input keyevent 223');
            results.push(`keyevent 223: ${wakeResult.trim() || 'ok'}`);
        } catch (e) {
            results.push(`keyevent 223 failed: ${(e as Error).message}`);
        }
